...
function gameTree(originBoard, color, maxScore, depth) {
    ...
    if (depth == 3) return originBoard.calcEval(color);
    var highScore = -1000000;
    for (var x = 0; x < boardSize; ++x) {
        for (var y = 0; y < boardSize; ++y) {
            var flipped = originBoard.getFlipCells(x, y, color);
            if (flipped.length > 0) {
    ...
    if (highScore == -1000000) return -gameTree(originBoard, !color, -highScore, depth + 1);
    return highScore;
}
